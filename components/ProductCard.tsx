    ...
    <div className="bg-white rounded-lg shadow-md overflow-hidden transform hover:-translate-y-1 hover:shadow-lg transition-all duration-300 flex flex-col">
      ...
      <div className="p-4 flex flex-col flex-grow">
        ...
        {seller && (
          <div className="flex items-center text-sm text-gray-500 mb-2">
            ...
            {seller.verificationStatus === VerificationStatus.VERIFIED && (
                <BadgeCheckIcon className="w-4 h-4 text-brand-secondary ml-1.5" title="Verified Seller" />
            )}
          </div>
        )}
        ...
        <div className="mt-auto flex justify-between items-center">
          <span className="text-xl font-bold text-brand-primary">₹{product.price.toLocaleString('en-IN')}</span>
          <button className="bg-brand-primary text-white font-bold py-2 px-4 rounded-lg hover:bg-opacity-90 transition-colors duration-200 text-sm">
            Add to Cart
          </button>
        </div>
      </div>
